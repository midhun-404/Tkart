...
const addOrderItems = async (req, res) => {
    try {
        ...
        if (orderItems && orderItems.length === 0) {
            ...
            // Fetch active deal to secure prices
            let activeDeal = null;
            try {
                ...
                console.error("Failed to fetch deal for checkout:", err);
            }
            ...
            for (const item of orderItems) {
                ...
                // Apply deal price if product is the active deal
                let price = product.selling_price;
                if (activeDeal && (activeDeal.productId === productId || activeDeal.product_id === productId)) {
                    price = Number(activeDeal.discountPrice || activeDeal.discount_price);
                    console.log(`[DEBUG] Applying deal price ₹${price} for product ${productId}`);
                }
