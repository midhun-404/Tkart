...
const DealManager = () => {
    ...
    const handleRemove = async () => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`${API_URL}/deals`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            setCurrentDeal(null);
            setSelectedProduct('');
            ...
            {currentDeal && (
                <div className="mb-8 p-4 bg-green-50 border border-green-100 rounded-lg">
                    ...
                    <div className="flex items-center gap-4">
                        <img src={currentDeal.productImage} alt="" className="w-16 h-16 object-cover rounded" />
                        <div>
                            <p className="font-bold">{currentDeal.productTitle}</p>
                            <p className="text-sm">Ends: {new Date(currentDeal.endTime).toLocaleString()}</p>
                            <p className="text-sm font-bold text-primary">Price: ₹{currentDeal.discountPrice}</p>
                        </div>
                        <button
