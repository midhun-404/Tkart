...
const DealSection = () => {
    ...
    useEffect(() => {
        ...
        const calculateTimeLeft = () => {
            ...
            if (difference <= 0) {
                // Option A: Hide it (current behavior, but flickers if close)
                // Option B: Show "Expired" state (better for UX)
                // For now, let's keep it visible but at 00:00:00 to verify it exists
                setTimeLeft({ hours: 0, minutes: 0, seconds: 0 });
                // Only hide if it's significantly past (e.g. 1 hour) or let the user see it's over
                ...
                <div className="md:w-1/2 space-y-8 text-center md:text-left">
                    ...
                    <div className="pt-4 flex flex-col items-center md:items-start">
                        <div className="flex items-end gap-4 mb-6">
                            <span className="text-4xl font-bold text-white">₹{deal.discountPrice}</span>
                            <span className="text-xl text-gray-500 line-through mb-1">₹{deal.originalPrice}</span>
                            <span className="bg-accent text-primary px-2 py-1 rounded text-xs font-bold mb-2">-{discountPercentage}%</span>
                        </div>
                        <Link to={`/product/${deal.productId}`} className="inline-flex items-center gap-2 px-8 py-4 bg-accent text-primary font-bold rounded-full hover:bg-yellow-400 transition-all shadow-lg shadow-accent/20">
